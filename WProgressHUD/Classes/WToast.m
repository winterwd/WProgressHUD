...
+ (UIImageView *)imageViewWith:(NSString *)imageName {
    ...
    imageview.image = [NSBundle w_loadImageFromResourceBundle:imageName];
    return imageview;
}
...
- (id)handleResponse:(id)responseJSON autoShowError:(BOOL)autouShowError {
    ...
    if (resultCode.integerValue != 0) {
        NSString *domain = [NSBundle w_localizedStringForKey:@"error"];
        error = [NSError errorWithDomain:domain code:resultCode.integerValue userInfo:responseJSON];
        if (autouShowError) {
